...
void Dense::compress(bool addClausesToLists, const char* newWhiteFile)
{
    ...
    for (Var v = 0 ; v < data.nVars(); v++) {
        ...
        // compress (remove) variable
            ...
            diff++;
            if (data.value(v) != l_Undef) {
                trail[v] = data.value(v);
                assert(mapping[v] == Compression::UNIT && "this variable has an assignment");
                ...
                assert(mapping[v] == Compression::UNIT && "variables that are not present in the formula, and that do not have a truth value, are treated as gap");
            }
        }
    }
    ...
    for (Var v = 0; v < data.nVars() ; v++) {
        if (!config.opt_dense_keep_assigned) {
            assert((data.doNotTouch(v) || data.value(v) == l_Undef) && "Assigned variable are not allowed");
        }

        // if( v+1 == data.nVars() ) cerr << "c final round with variable " << v+1 << endl;

        const Var compressed = compression.importVar(v);

        // the variable was not removed from the formula
        if (compressed != var_Undef) {
            DOUT(if (config.dense_debug_out) {
            cerr << "c map " << (v + 1) << " to " << (compressed + 1) << endl;
            });
            // if( v+1 == data.nVars() ) cerr << "c final round with move" << endl;
            // cerr << "c move intern v " << v << " to " << compression.mapping[v] << endl;

            // map variable, and if done, re-organise everything
            data.moveVar(v, compressed, v + 1 == data.nVars());
        ...
        // variable was removed by compression (that means the variable is a unit)
            ...
            if (v + 1 == data.nVars()) {
                data.moveVar(v - diff, v - diff, true);
            }
        }
    }
